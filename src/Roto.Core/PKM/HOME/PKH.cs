...
using static Roto.Core.GameVersion;
    ...
    IRibbonSetRibbons, IRibbonSetMarks, IAppliedMarkings7
{
    ...
    public PKH() : base(HomeCrypto.SIZE_STORED)
    {
        ...
        var mem = Raw[(HomeCrypto.SIZE_1HEADER + 2)..];
        var core = mem[..CoreDataSize];
        Core = new GameDataCore(core) { AffixedRibbon = Roto.Core.AffixedRibbon.None };
    }
    ...
    public static PKH ConvertFromPKM(PKM pk)
    {
        ...
        if (blank.Species is (int)Roto.Core.Species.Arceus or (int)Roto.Core.Species.Silvally)
            blank.Form = 0;
        return blank;
    }
